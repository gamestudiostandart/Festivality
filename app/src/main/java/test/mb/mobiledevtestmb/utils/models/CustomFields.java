...
public class CustomFields {
    ...
    public ArrayList<UserField> getList(){
        ...
        if(getFirstName() != null ||  getFirstName().length() != 0){list.add(new UserField("First Name", getFirstName()));}
        if(getLastName() != null ||  getLastName().length() != 0){list.add(new UserField("Last_name", getLastName()));}
        if(getPosition() != null ||  getPosition().length() != 0){list.add(new UserField("Position", getPosition()));}
        if(getCompany() != null ||  getCompany().length() != 0){list.add(new UserField("Company", getCompany()));}
        if(getCompanySize() != null ||  getCompanySize().length() != 0){list.add(new UserField("Company size", getCompanySize()));}
        if(getPhone() != null ||  getPhone().length() != 0){list.add(new UserField("Phone", getPhone()));}
        if(getEmail() != null ||  getEmail().length() != 0){list.add(new UserField("Email", getEmail()));}
        if(getPublicEmail() != null ||  getPublicEmail().length() != 0){list.add(new UserField("Public email", getPublicEmail()));}
        if(getCity() != null ||  getCity().length() != 0){list.add(new UserField("City", getCity()));}
        list.add(new UserField("Age", getAge() == 0?null:String.valueOf(getAge())));

        if(getAttendeeProviding() != null ||  getAttendeeProviding().length() != 0){list.add(new UserField("Attendee providing", getAttendeeProviding()));}
        if(getAttendeeLookingFor() != null ||  getAttendeeLookingFor().length() != 0){list.add(new UserField("Attendee looking for", getAttendeeLookingFor()));}
        if(getPositionType() != null ||  getPositionType().length() != 0){list.add(new UserField("Position type", getPositionType()));}
        if(getAttendeeType() != null ||  getAttendeeType().length() != 0){list.add(new UserField("Attendee type", getAttendeeType()));}
        if(getIndustryTags() != null ||  getIndustryTags().length() != 0){list.add(new UserField("Industry tags", getIndustryTags()));}
        if(getIndustryComplimentaryTags() != null ||  getIndustryComplimentaryTags().length() != 0){list.add(new UserField("Industry complimentary tags", getIndustryComplimentaryTags()));}
        return list;
    }
