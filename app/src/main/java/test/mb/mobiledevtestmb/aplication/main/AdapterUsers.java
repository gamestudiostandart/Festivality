...
import test.mb.mobiledevtestmb.aplication.details.ActivityUserPage;
import test.mb.mobiledevtestmb.utils.models.User;
import test.mb.mobiledevtestmb.utils.CircleTransform;
...
public class AdapterUsers extends RecyclerView.Adapter<AdapterUsers.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull final AdapterUsers.ViewHolder holder, final int position) {
        ...
        if(user.getCustomFields() != null){
            if (!TextUtils.isEmpty(user.getCustomFields().getFullName())) {
                holder.tv_name.setText(user.getCustomFields().getFullName());
            } else {
                holder.tv_name.setText(String.valueOf(user.getId()));
            }
            ;
            holder.tv_description.setText(String.valueOf(user.getCustomFields().getAge()));
        } else {
            holder.tv_name.setText(String.valueOf(user.getId()));
