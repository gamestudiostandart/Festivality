...
import test.mb.festivality.utils.parser.convert.usertypestostring.UserTypeToStringConvertImpl;
import test.mb.festivality.utils.parser.convert.usertypestostring.UserTypeToStringConverter;
...
public class User {
    ...
    public ArrayList<UserField> getList(){
        ...
        list.add(new UserField(MyApp.getContext().getResources().getString(R.string.featured), String.valueOf(getIsFeatured())));
        if(getChildren() != null &&  getChildren().length() != 0){list.add(new UserField(MyApp.getContext().getResources().getString(R.string.children), getChildren()));}
        if(getType() != null &&  getType().length() != 0){list.add(new UserField(MyApp.getContext().getResources().getString(R.string.type), getType().replaceAll(", ", "\nn")));}
        list.add(new UserField(MyApp.getContext().getResources().getString(R.string.likes), String.valueOf(getLikes())));
        return list;
    }
    ...
    public User(JSONObject jsonObject, GetDataListener listener) {
        int id = 0;
        try {
